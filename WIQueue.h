...
namespace GA {
    ...
    class WIQueue {
        struct slot {
            enum STATE : unsigned char { FREE = 0U, WRITING = 1U, READY = 2U, READING = 3U };
...
template<class T>
GA::WIQueue<T>::WIQueue() : mData(), mHead(0U), mTail(0U)  {
}
...
void GA::WIQueue<T>::Push(const std::shared_ptr<T> & value) {
    ...
    if(mData[target].state != slot::STATE::FREE) { //slot is not ready yet
        ...
        mData[target].conVar.wait(lock, mData[target].IsFree);
    }
...
std::shared_ptr<T> GA::WIQueue<T>::Pop() {
    ...
    if(mData[target].state != slot::STATE::READY) { //slot is not ready yet
        std::unique_lock<std::mutex> lock(mData[target].mutex);
        mData[target].conVar.wait(lock, mData[target].IsReady);
    }
